...
import {
	getTumblrService,
	TumblrService,
} from "../../services/tumblr-service.js";
...
const params = z.object({
	...
	image: z
		.string()
		.optional()
		.describe("A public image URL, or base64-encoded image bytes to attach."),
	imageContentType: z
		.string()
		.optional()
		.default("image/jpeg")
		.describe("MIME type, required when `image` is base64-encoded bytes."),
	state: z
	...
	blogIdentifier: z
		.string()
		.optional()
		.describe("Blog name or URL (overrides TUMBLR_BLOG_IDENTIFIER env var)"),
	accessToken: z
		.string()
		.optional()
		.describe("Tumblr OAuth token (overrides TUMBLR_ACCESS_TOKEN env var)"),
});
		...
		try {
			const service = p.accessToken
				? new TumblrService({
						...
						blogIdentifier: p.blogIdentifier,
					})
				: getTumblrService();
			...
			const content: Array<{
				...
				text?: string;
				subtype?: string;
				media?: { url?: string; identifier?: string };
			}> = [{ type: "text", text: p.text }];

			const isUrl =
				p.image?.startsWith("http://") || p.image?.startsWith("https://");
			if (p.image && isUrl) {
				content.push({ type: "image", media: { url: p.image } });
			} else if (p.image) {
				content.push({ type: "image", media: { identifier: "image0" } });
			}
			...
			const result = await service.createPost(
				...
				p.image && !isUrl
					? [
							{
								content: p.image,
								contentType: p.imageContentType ?? "image/jpeg",
								identifier: "image0",
							},
						]
					: undefined,
